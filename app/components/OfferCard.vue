...
<template>
  <article
    ...
    <div class="flex justify-between items-center gap-3 mb-3.5 md:mb-4">
      <span class="text-xs text-muted flex items-center gap-1.5 md:text-xs"><span class="text-sm" aria-hidden="true">{{
        sportIcons[offer.sport] }}</span>{{ offer.sport }}</span><span class="text-xs text-mint md:text-xs">{{
            dateLabel(offer.date) }}</span>
    </div>
    <h3 class="text-base leading-tight font-semibold tracking-tight md:text-sm">
      {{ offer.event }}
    </h3>
    <p class="whitespace-pre-line wrap-anywhere text-sm text-mint leading-relaxed mt-2 min-h-0 mb-1 md:text-xs md:mb-0">
      {{ offer.market }}
    </p>
    <div
      ...
      <span class="text-lime text-xs tracking-tight font-bold flex items-center gap-0.5 md:text-xs">SUPER<span
          class="text-white">QUOTA</span>
        <AppIcon name="boost" :size="17" />
      </span>
      <div class="flex items-center gap-1.5 shrink-0 ml-auto xl:ml-0">
        ...
        <AppIcon class="text-foreground" name="boost" :size="17" /><strong
          class="text-2xl leading-tight tracking-tight md:text-2xl">{{ quota(offer.boosted_odds) }}</strong>
      </div>
    </div>
    <div class="px-0 py-3 flex items-center justify-between mt-3 border-t border-t-[#2d3b30] gap-2.5 md:mt-3">
      <StatusBadge :outcome="offer.outcome" /><span
        ...
        v-if="offer.outcome !== 'pending'" :class="[net < 0 ? 'text-negative' : 'text-mint']">{{ money(net, true) }}</span><span v-else
        class="text-xs text-muted">Da aggiornare</span>
    </div>
  </article>
</template>
