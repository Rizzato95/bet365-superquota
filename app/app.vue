...
<template>
  <div>
    <a class="p-3 fixed top-[-80px] left-5 bg-mint text-[#102019] z-100 focus:top-2.5" href="#main">Vai al contenuto</a>
    <header
      ...
      <NuxtLink to="/management"
        ...
        <AppIcon :name="session?.admin ? 'settings' : 'lock'" :size="16" /><span
          class="hidden md:inline">Management</span>
      </NuxtLink>
    </header>
    <aside
      ...
      <nav>
        <NuxtLink
          ...
          <AppIcon :name="item.icon" /><span>{{ item.name }}</span><span v-if="route.path === item.to"
            class="w-[5px] h-[5px] bg-mint rounded-full ml-auto" />
        </NuxtLink>
      </nav>
      <template v-if="session?.admin">
        ...
        <NuxtLink
          class="px-[15px] py-3.5 flex items-center gap-3 text-[#99a99d] rounded-[7px] text-sm mb-[5px] transition-colors duration-150 aria-[current=page]:bg-[#21392c] aria-[current=page]:text-[#69dfae] hover:bg-[#24372a] hover:text-[#d1e7d7]"
          to="/management" :aria-current="route.path.startsWith('/management') ? 'page' : undefined">
          <AppIcon name="settings" /><span>Management</span>
        </NuxtLink>
      </template>
      <div class="px-4 mt-auto pt-10 pb-1.5">
        <span
          class="block border-t border-t-[#2a362d] mt-6.5 pt-[19px] text-[10px] leading-[1.6] text-[#65756a]">Progetto
          indipendente da bet365</span>
      </div>
    </aside>
    <main id="main"
      ...
      <footer
        ...
        <span class="first:text-[#8d9f93]">{{ config.brand.name }}</span><span class="first:text-[#8d9f93]">Risultati
          storici · Simulazioni teoriche a puntata fissa</span>
      </footer>
    </main>
    <nav
      ...
      <NuxtLink
        ...
        v-for="item in navigation" :key="item.to" :to="item.to"
        :aria-current="route.path === item.to ? 'page' : undefined">
        <AppIcon :name="item.icon" :size="21" /><span>{{ item.name }}</span>
      </NuxtLink>
    </nav>
  </div>
</template>
